...
export default function GrantsPage() {
  ...
  useEffect(() => {
    ...
    listGrants({ page, pageSize, sort, q })
  ...
  if (!rows.length) return <EmptyState title="No grants found." hint="Try clearing filters." />
  ...
  return (
    <section className="container py-10 grid gap-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {rows.map((row) => (
          <Card key={row.id}>
            <CardHeader><CardTitle><Link to={`/grants/${row.slug}`} className="underline text-primary">{row.title}</Link></CardTitle></CardHeader>
            <CardContent className="text-sm text-gray-700 space-y-1">
              {'year' in row && row.year ? <div><span className="font-medium">Year:</span> {row.year}</div> : null}
              {row.amount ? <div><span className="font-medium">Amount:</span> {row.amount}</div> : null}
            </CardContent>
          </Card>
        ))}
      </div>
