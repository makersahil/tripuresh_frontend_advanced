...
import { listPublications } from './api'
...
export default function PublicationsPage() {
  ...
  useEffect(() => {
    ...
    listPublications({ page, pageSize, sort, q })
  ...
  if (!rows.length) return <EmptyState title="No publications found." hint="Try clearing filters." />
  ...
  return (
    <section className="container py-10 grid gap-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {rows.map((row) => (
          <Card key={row.id}>
            <CardHeader><CardTitle><Link to={`/publications/${row.slug}`} className="underline text-primary">{row.title}</Link></CardTitle></CardHeader>
            <CardContent className="text-sm text-gray-700 space-y-1">
              {'year' in row && row.year ? <div><span className="font-medium">Year:</span> {row.year}</div> : null}
              {row.publisher ? <div><span className="font-medium">Publisher:</span> {row.publisher}</div> : null}
              {row.type ? <div><span className="font-medium">Type:</span> {row.type}</div> : null}
            </CardContent>
          </Card>
        ))}
      </div>
