...
export default function ArticlesPage() {
  ...
  useEffect(() => {
    ...
    listArticles({ page, pageSize, sort, q })
  ...
  if (!rows.length) return <EmptyState title="No articles found." hint="Try clearing filters." />
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {rows.map((row) => (
          <Card key={row.id}>
            <CardHeader>
              <CardTitle>
                <Link to={`/articles/${row.slug}`} className="underline text-primary">{row.title}</Link>
              </CardTitle>
            </CardHeader>
            <CardContent className="text-sm text-gray-700 space-y-1">
              {'year' in row && row.year ? <div><span className="font-medium">Year:</span> {row.year}</div> : null}
              {row.journal ? <div><span className="font-medium">Journal:</span> {row.journal}</div> : null}
            </CardContent>
          </Card>
        ))}
      </div>
